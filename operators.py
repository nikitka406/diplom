...
def Relocate(x_start, y_start, s_start, a_start, target_function_start, sizeK_start, iteration, timeLocal,
             evolution=2, cycle=factory.param_local_search):
    ...
    fileflag = 0
    it = 0
    while it < cycle and ((evolution == 1 and (time.time() - start) < factory.timeAlgStart)
                          or (evolution == 2 and (time.time() - start) < factory.timeAlg)):
        it += 1
                        ...
                        for i2 in range(1, factory.N):
                            ...
                            if (client != sosedLeft and client != sosed and client != sosedRight
                                ...
                                if (Y[sosed][sosedK] == 1 and sosed != 0) or (
                                    ...
                                    if ((evolution == 1 and (time.time() - start) < factory.timeAlgStart)
                                            or (evolution == 2 and (time.time() - start) < factory.timeAlg)):
                                        ...
                                        file.write(
                                            "Целевая функция последнего стартового решения = " + str(
                                        ...
                                        if evolution == 1:
                                            ...
                                            if (minimum2 == target_function and target_function != -1) or (
                                                    fileflag == 1 and it == 0):
                                                file.write(
                                                    "Новое перемещение, лучше чем стартовое, сохраняем это решение" + '\n')
                                                file.write("Новая целевая функция равна " + str(target_function) + '\n')
                                                ...
                                                file.write(
                                                    "Старая целевая функция равна " + str(target_function_start) + '\n')
...
def Two_Opt(x_start, y_start, s_start, a_start, target_function_start, sizeK_start, iteration, timeLocal):
    ...
    while it < factory.param_local_search and (time.time() - start) < factory.timeAlg:
        it += 1
                        ...
                        for i2 in range(1, factory.N):
                            ...
                            if (Y[client2][client2Car] == 1 and client2 != 0) or (
                                ...
                                IsContainTailInStart(sequenceX2[client2Car], tail1, client2, file)
                                    ...
                                    if (time.time() - start) < factory.timeAlg:
        ...
        for i1 in range(1, factory.N):
            ...
            if Ystart[client][k] == 1:
                if Astart[client][k] + Sstart[client][k] > factory.l[client]:
                    file.write(
                        "Нашли объект который опаздывает " + str(client) + " который обслуживает машина " + str(
                            k) + '\n')
                                                ...
                                                if (Y[sosed][sosedK] == 1 and sosed != 0) or (
                                                    ...
                                                    file.write(
                                                        "Попробую одну скважину с объекта " + str(
                                                            ...
                                                            k) + "\n")
                                                    file.write(
                                                        " отдать машине " + str(sosedK) + " рядом с объектом " + str(
                                                            sosed) + "\n")

                                                    x, y, s, a, target_function, sizeK = OperatorJoinFromHelp(X, Y,
                                                                                                              Sresh,
                                                                                                              A,
                                                                                                              SizeK,
                                                                                                              client, k,
...
def Exchange(x_start, y_start, s_start, a_start, target_function_start, sizeK_start, iteration, timeLocal,
    ...
    it = 0
    # for it in range(factory.param_local_search):
    while it < cycle and (time.time() - start) < factory.timeAlg:
        it += 1
                                    ...
                                    for i in range(len(subseq1)):
                                        if subseq1[-1] != 0:
                                            ...
                                            for j in range(len(subseq2)):
                                                ...
                                                if sequenceX2[clientCar][seq1End + 1] == 0 or sequenceX2[clientCar][
                                                    seq1End] == 0:
                                                    sequence1Right = 0
                                                else:
                                                ...
                                                if sequenceX2[sosedCar][seq2End + 1] == 0 or sequenceX2[sosedCar][
                                                    seq2End] == 0:
                                                    sequence2Right = 0
                                                else:
                                                ...
                                                if (not IsContainTailInStart(sequenceX2[sosedCar], buf1,
                                                                             sequence2Left,
                                                                             file)
                                                    ...
                                                    and not IsContainTailInEnd(sequenceX2[clientCar], buf2,
                                                        ...
                                                        or (
                                                        clientCar == sosedCar and not IsContainTailInTail(buf1,
                                                                                                          buf2,
                                                                                                          file)):
                                                    ...
                                                    if (time.time() - start) < factory.timeAlg:
                                                        if ResultCoins():
                                                            ...
                                                            x, y, s, a, target_function, sizeK = OperatorJoinFromExchange(
                                                                X, Y, Sresh, A, SizeK, TargetFunction, sequenceX2,
                                                                client,
                                                        ...
                                                        if (minimum2 == target_function and target_function != -1) or (
                                                            ...
                                                            file.write(
                                                                "Новое перемещение, лучше чем стартовое, сохраняем это решение" + '\n')
                                                            file.write("Новая целевая функция равна " + str(
                                                                target_function) + '\n')
                                                            ...
                                                            file.write(
                                                                "Новое перемещение, хуже чем последние добавленое стартовое решение" + '\n')
                                                            file.write("Старая целевая функция равна " + str(
                                                                target_function_start) + '\n')
...
def PopulationOfSolutions(Target_Function, SizeSolution, iteration, timeLocal):
    for n in range(factory.param_population):  # создаем популяцию решений в кол-ве param_population
        ...
        x, y, s, a, Target_Function[n], SizeSolution[n], timeLocal[0] = Relocate(X, Y, Sresh, A,
                                                                                 Target_Function[n],
