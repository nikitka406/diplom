...
def OneCarOneLocation(N, KA, S, t, wells):
    # копия переменной задачи Х, только кол-вo машин = число локаций
    x = [[[0 for k in range(KA)] for j in range(N)] for i in range(N)]
    y = [[0 for k in range(KA)] for i in range(N)]  # посещает или нет ТС с номером К объект i
    for k in range(N):
        y[0][k] = 1
    s = [[0 for k in range(KA)] for i in range(N)]  # время работы ТС номером К на объекте i
