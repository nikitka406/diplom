...
param_local_search = 5  # параметр который будет показывать, сколько раз будем переставлять клиента(локадьный поиск)
population = 5  # параметр который будет показывать, кол-во особей в популяции
min_num_cl_in_car = 2  # параметр отвечающий за, сколько минимально клиентов может присутствовать в однрой машине
param_crossing = population * 2
