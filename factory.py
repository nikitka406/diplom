...
param_population = 2#int(N/2)  # параметр который будет показывать, кол-во особей в популяции
param_min_num_cl_in_car = 8  # параметр отвечающий за, сколько минимально клиентов может присутствовать в однрой машине
param_crossing = 1#param_population * 3  # сколько раз будем скрещивать решения
...
param_local_search = int(N)  # сколько раз будем запускать локальный поиск
